...
class ColorNet(nn.Module):
    def __init__(self, config, input_ch=4, geo_feat_dim=15, 
        ...
        self.model = self.get_model(config['mapper']['decoder']['tcnn_network'])
    ...
    def get_model(self, tcnn_network=False):
        if tcnn_network:
            rospy.logdebug('Color net: using tcnn')
            return tcnn.Network(
...
class SDFNet(nn.Module):
    def __init__(self, config, input_ch=3, geo_feat_dim=15, hidden_dim=64, num_layers=2):
        ...
        self.model = self.get_model(tcnn_network=config['mapper']['decoder']['tcnn_network'])
    ...
    def get_model(self, tcnn_network=False):
        if tcnn_network:
            rospy.logdebug('SDF net: using tcnn')
            return tcnn.Network(
...
class ColorSDFNet(nn.Module):
    ...
    def __init__(self, config, input_ch=3, input_ch_pos=12):
        ...
        self.color_net = ColorNet(config, 
                ...
                geo_feat_dim=config['mapper']['decoder']['geo_feat_dim'], 
                hidden_dim_color=config['mapper']['decoder']['hidden_dim_color'], 
                num_layers_color=config['mapper']['decoder']['num_layers_color'])
        self.sdf_net = SDFNet(config,
                input_ch=input_ch+input_ch_pos,
                geo_feat_dim=config['mapper']['decoder']['geo_feat_dim'],
                hidden_dim=config['mapper']['decoder']['hidden_dim'], 
                num_layers=config['mapper']['decoder']['num_layers'])
